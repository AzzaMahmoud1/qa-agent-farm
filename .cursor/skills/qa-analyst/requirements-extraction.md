---
name: qa-analyst-requirements-extraction
description: >-
  Extract requirements (goal, pre/post-conditions, flows, business rules,
  messages EN/AR, UI specs) from a Jira story and write the Atomic
  Requirements Checklist to <OUTPUT_DIR>/<ISSUE_ID>-requirements.md. Use
  when the analyst needs to extract/break down a story into that file.
---

# Requirements extraction (Analyst)
...
Write the breakdown to `<OUTPUT_DIR>/<ISSUE_ID>-requirements.md` using this template (create the folder if it doesn't exist):
...
Return the path to the written file and a one-line summary of what was extracted (counts of AF/EF/BR/MSG/DM found, total atomic checklist items, and how many requirements were updated or added based on comments vs. the description alone).
