 ...
 * Deterministic output aligned with the Analyst contract (agents/analyst-contract.js).
...
/**
 * Observable pass/fail from the AC text itself — never invent HTTP codes or
 * generic "success UI state" boilerplate.
 */
function outcomeFromAcText(acText) {
  let body = String(acText || "").trim().replace(/\s+/g, " ").replace(/\.+$/, "");
  if (!body) return "";
  body = body
    .replace(/^(verify|ensure|confirm)\s+that\s+/i, "")
  ...
  body = body.trim();
  if (!body) return String(acText || "").trim();
  return body.charAt(0).toUpperCase() + body.slice(1);
}

function inferPassFailEvidence(acText) {
  const outcome = outcomeFromAcText(acText);
  if (!outcome) return { pass: "", fail: "" };
  return {
    pass: outcome,
    fail: `Observed behavior does not match: ${outcome}`,
  };
}

/** Why this AC is independently testable / what evidence to observe (Author-style, not Playwright). */
function reasonForCondition(entry, evidence) {
  const source = String(entry?.source || "AC").trim() || "AC";
  const outcome = String(evidence?.pass || outcomeFromAcText(entry?.text) || "").trim();
  const t = String(entry?.text || "").toLowerCase();
  let how = "observable outcome from ticket text";
  if (/\bapi|endpoint|http|status code|response\b/.test(t)) {
    how = "API status/response fields as documented (or REST convention if unspecified)";
  } else if (/\blog\s*in|sign\s*in|button|screen|ui|display|page|form|label\b/.test(t)) {
    how = "UI state / visible controls on the relevant screen";
  } else if (/\bmessage|error|toast|notification\b/.test(t)) {
    how = "exact message/copy shown to the user";
  } else if (/\bconfig|flag|deploy|enable|disable\b/.test(t)) {
    how = "configuration effect without requiring a code invent";
  }
  if (!outcome) {
    return `Sourced from ${source}; independently checkable — ${how}`;
  }
  return `Sourced from ${source}; independently checkable — ${how}. Pass when: ${outcome}`;
}
...
function buildAnalystOutput(story) {
  ...
  validEntries.forEach((entry, i) => {
    ...
    if (isShallowTestAction(entry.text, action.summary)) {
    ...
    testable_conditions.push({
      ...
      reason: reasonForCondition(entry, evidence),
  ...
  const includedReasons = testable_conditions.map((c) =>
    `${c.id}: ${c.reason || `Included from ${c.source || "AC"} — ${c.pass_evidence || c.ac_text}`}`
  );
  const evidencePlan = testable_conditions.length
    ? testable_conditions.slice(0, 5).map((c) =>
      `${c.id}: observe "${c.pass_evidence || c.ac_text}" (${c.source || "AC"})`
    )
    : ["No testable conditions — cannot form an evidence plan until ACs are clarified"];
  const confidenceOverall = testable_conditions.length && !ambiguous_acs.length
    ? "high"
    : (testable_conditions.length ? "medium" : "low");
  const confidenceReason = !testable_conditions.length
    ? "zero testable ACs after disposition"
    : (ambiguous_acs.length ? "vague wording remains on one or more ACs" : "dispositions grounded in ticket text");
  ...
  return {
    ...
    analyst_reasoning: {
      ...
      rejected_detail: allRejected.length ? allRejected : ["None"],
      evidence_plan: evidencePlan,
      confidence: { overall: confidenceOverall, reason: confidenceReason },
      unimplemented_rules,
      ambiguous_acs,
      open_questions: ambiguous_acs.map((a) => a.question_for_human || a.issue).filter(Boolean),
    },
    testable_conditions,
    prerequisites_needed,
    coverage_gaps,
    ...
    analyst_report: {
      what_i_did: [
        ...
        "Mapped testable conditions with pass/fail evidence and per-AC reason",
        "Recorded included vs rejected with reasons (no silent drops)",
      ...
      why: [
        {
          ...
          reason: ready_for_test_design
            ? `${testable_conditions.length} AC(s) with reasons; access/env gaps do not block design`
        ...
        ...testable_conditions.slice(0, 8).map((c) => ({
          decision: `Include ${c.id}`,
          reason: c.reason,
          impact_if_wrong: "Writer may drop or mis-state the assertion",
        })),
        ...rejected_strings.slice(0, 5).map((line) => ({
          decision: "Reject non-AC / shallow line",
          reason: line,
          impact_if_wrong: "Noise TCs or false coverage",
        })),
      ],
      orchestrator_actions,
      confidence: {
