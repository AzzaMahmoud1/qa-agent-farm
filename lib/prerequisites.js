...
function validateAnalystOutput(story, analystOutput) {
  ...
  // MAIN GATE — keep in sync with agents/analyst-contract.js (Validator second opinion)
  ...
  if (hasProceed && missingBlocking.length) {
    failures.push(`MAIN GATE: PROCEED while ${missingBlocking.length} blocking prerequisite(s) still missing`);
    failedRules.add(ruleMainGate);
  }
  if (analystOutput?.ready_for_test_design === true && missingBlocking.length) {
    failures.push("MAIN GATE: ready_for_test_design true while blocking prerequisites are missing");
    failedRules.add(ruleMainGate);
  }
  if (missingBlocking.length && !blockingActs.length) {
