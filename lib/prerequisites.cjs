// lib/prerequisites.cjs is AUTO-GENERATED from lib/prerequisites.js by
// `npm run sync:cjs` — edit prerequisites.js only, never edit the .cjs.
/**
 * Story-first prerequisite analysis (Analyst L2 stub / simulator).
 * Deterministic output aligned with agent1_requirement_analyst_v3.md.
 * Live Cursor Analyst follows the prompt; this stub approximates the schema.
 */

const AC_SOURCE_SECTIONS = new Set(["business_rules", "alternative_flow", "exception_flow", "ac", "ac_inline"]);
 ...
 * Duplicated from agents/analyst-contract.js (this file is also a classic browser
 * script / .cjs — cannot import ESM). Keep in sync with the export there.
 */
function isDesignBlockingPrereq(b) {
...
function isUseCaseSectionHeader(line) {
  ...
  return /^(post-conditions?|pre-conditions?|preconditions?|prerequisites?|basic flow|alternative flows?|exception flows?|main flow|main success scenario|extensions?|actors?|trigger|primary actor|secondary actors?|success scenario|failure scenario|business rules?|special requirements?|assumptions?|notes?|flow of events|sub-flows?|data table|used api|performance metrics?|acceptance criteria?|acs?|rules?|requirements?|constraints?|definition of done|dod|edge cases?|error flow|scenarios?|given[\s/]?when[\s/]?then)\s*:?\s*$/i.test(t);
}

// Heading text → canonical AC-source section, matched by *meaning* not exact
// string (rule 1a/1b in the Analyst prompt). New synonyms extend these two
// maps instead of widening a single flat allowlist.
const RULE_BEARING_HEADINGS = /^(business rules?|rules?|requirements?|constraints?|definition of done|dod)$/;
const FLOW_BEARING_HEADINGS = /^(alternative flows?|edge cases?|scenarios?|given[\s/]?when[\s/]?then)$/;
const EXCEPTION_BEARING_HEADINGS = /^(exception flows?|error flow)$/;
...
function getSectionType(line) {
  ...
  if (EXCEPTION_BEARING_HEADINGS.test(t)) return "exception_flow";
  if (FLOW_BEARING_HEADINGS.test(t)) return "alternative_flow";
  if (RULE_BEARING_HEADINGS.test(t)) return "business_rules";
  ...
  if (/^acceptance criteria?$|^acs?$/.test(t)) return "ac";
...
function acSourceLabel(section) {
  ...
  if (section === "ac_inline") return "Acceptance Criteria (inline marker)";
  return null;
}
...
function isDataTableConstraint(line) {
  const t = String(line || "").trim();
  return /^(field|column|attribute)\s*:/i.test(t)
    || /\b(max length|min length|data type|format|regex|nullable|required field)\b/i.test(t)
    || /^\w[\w\s]*\s*[-–]\s*(string|number|boolean|date|enum)/i.test(t);
}

function isStrongAcceptanceCriterion(line, section) {
  const t = String(line || "").trim();
  if (!t || isMetadataLine(t) || isNarrativeContext(t)) return false;
  // An explicit "AC:" / "Acceptance Criteria:" label (heading or inline, rule
  // 1c) is the strongest possible signal of intent — trust it instead of
  // re-litigating wording against the modal-verb heuristic below, which is
  // tuned for content only *inferred* to be a rule from its section.
  if (section === "ac" || section === "ac_inline") return true;
  if (section === "data_table" && isDataTableConstraint(t) && !/\b(must|should|shall|may|reject|allow|deny)\b/i.test(t)) {
    return false;
  }
  ...
  if (/\b(expires? after|within \d+\s*(minute|hour|day|second)s?|valid for \d+)\b/i.test(t)) return true;
  if (/\b(customer|admin|user|manager|role)\s+may\b/i.test(t)) return true;
  if (/\b(may view only|only their own|must not access|shall not|may not)\b/i.test(t)) return true;
  ...
  if (/\b(may|may not)\b/i.test(t) && /\b(view|access|refund|create|update|delete|approve|reject)\b/i.test(t)) {
    return true;
  }
...
// Rule 1c: an explicit inline label makes a line an AC candidate regardless of
// which section (or no section at all) it sits in. Distinct from a *heading*
// line ("AC:" alone on its own line, handled by getSectionType) — this matches
// "AC: <criterion text>" on a single line, anywhere in the ticket body.
const INLINE_AC_MARKER_RE = /^(ac(-\d+)?|acceptance criteria|rule|scenario)\s*:\s*(.+)$/i;

function getInlineAcMarker(line) {
  const t = String(line || "").trim();
  const m = t.match(INLINE_AC_MARKER_RE);
  if (!m || !m[3]?.trim()) return null;
  return m[3].trim();
}

// Rule 1 fallback: unlabeled free text is never promoted directly to an AC —
// but a line with the *system* (not a human actor) as the subject of a
// normative/conditional statement shouldn't be silently discarded either.
// Returns true so the caller can route it to `ambiguous_acs` instead of
// invention or a silent drop.
const HUMAN_ACTOR_SUBJECT_RE = /^(the\s+)?(user|users|admin|admins|administrator|customer|customers|agent|agents|actor|actors|manager|managers|owner|owners|operator|operators)\b/i;

function hasSystemNormativeStructure(line) {
  const t = String(line || "").trim();
  if (!t) return false;
  if (HUMAN_ACTOR_SUBJECT_RE.test(t)) return false;
  if (isFlowOrScenarioLine(t)) return false;
  if (/^(if|when|given)\b/i.test(t)) return true;
  if (/^only\s+[\w\s,]+?\b(may|can|shall|is allowed to|are allowed to)\b/i.test(t)) return true;
  if (/\b(must|must not|shall|shall not|cannot|can't|will not|is limited to|are limited to|rejects?|redacts?|denies?|prevents?)\b/i.test(t)) return true;
  return false;
}

const UNMARKED_SYSTEM_RULE_REASON = "reads like a system rule (normative/conditional structure) but is not under a recognized AC section or inline marker — confirm if intended as an AC";
...
function sanitizeAcceptanceCriteria(acList, opts = {}) {
  ...
  for (const entry of entries) {
    ...
    if (!AC_SOURCE_SECTIONS.has(section)) {
      rejected.push({
        text,
        reason: section === "inferred_business_rules"
          ? "not under Business Rules / Alternative Flow / Exception Flow / Acceptance Criteria — do not invent ACs from free text"
          : `section "${section}" is not an AC source`,
      });
      continue;
    }
...
function isShallowTestAction(acText, actionSummary, section) {
  if (!actionSummary || !actionSummary.startsWith("verify: ")) return false;
  if (isMetadataLine(acText) || !isLikelyAcceptanceCriterion(acText, section || "business_rules")) return true;
...
function buildCoverageGaps(story, acCount) {
  ...
  const gapTemplates = [
    {
      when: () => !/\b(unicode|rtl|arabic|locale|i18n|l10n)\b/.test(corpus),
    ...
    {
      when: () => !/\b(rate limit|throttle|retry|timeout|backoff)\b/.test(corpus),
    ...
    {
      when: () => /\b(admin|role|permission|tenant|customer|may view|only their own)\b/.test(corpus),
      gap: "Authorization negative paths need explicit denial scenarios",
      category: "security",
      severity: "blocking",
      suggested_test: "Access resource with wrong role and expect denial with audit log",
    },
    {
      when: () => !/\b(admin|role|permission|tenant|customer|may view|only their own)\b/.test(corpus),
    ...
    {
      when: () => !/\b(concurrent|parallel|simultaneous|lock|race)\b/.test(corpus),
    ...
    {
      when: () => !/\b(webhook|notification|billing|queue|event|integration)\b/.test(corpus),
    ...
    {
      when: () => !/\b(regression|related|adjacent|module)\b/.test(corpus),
    ...
    {
      when: () => !/\b(performance|latency|load|p95|throughput|sla)\b/.test(corpus),
    ...
    {
      when: () => !/\b(ui|layout|screen|button|accessibility|a11y|rtl)\b/.test(corpus),
  ...
  const gaps = gapTemplates.filter((t) => t.when()).map(({ gap, category, severity, suggested_test }) => ({
    gap,
    category,
    severity,
    suggested_test,
...
function legacyPrereqAdapter(prerequisites_needed, testable_conditions, reasoning_steps) {
  ...
  blocking.forEach((p, i) => {
    const entry = {
      ...
      expected_shape: p.expected_shape,
...
function parseFullRequirements(text) {
  ...
  function rejectLine(line, reason, meta = {}) {
    const t = String(line || "").trim();
    if (!t) return;
    if (!rejected.some((r) => r.text === t)) {
      const entry = { text: t, reason };
  ...
  function pushAcCandidate(line, sourceSection) {
    ...
    if (!AC_SOURCE_SECTIONS.has(sourceSection)) {
      rejectLine(
        stripped,
        sourceSection === "inferred_business_rules"
          ? "not under Business Rules / Alternative Flow / Exception Flow / Acceptance Criteria — do not invent ACs from free text"
          : `section "${sourceSection}" is not an AC source`,
      );
      return;
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Rule 1c: an inline AC/Acceptance Criteria/Rule/Scenario marker makes this
    // line an AC candidate regardless of which section it physically sits in.
    const inlineAcText = getInlineAcMarker(trimmed);
    if (inlineAcText) {
      pushAcCandidate(inlineAcText, "ac_inline");
      continue;
    }
    ...
    // Rule 1 fallback: unlabeled text with the system (not a human actor) as
    // subject of a normative/conditional statement — flag, don't invent or drop.
    const candidateText = stripListPrefix(trimmed);
    if (hasSystemNormativeStructure(candidateText)) {
      rejectLine(candidateText, UNMARKED_SYSTEM_RULE_REASON);
      descriptionLines.push(lines[i]);
      continue;
    }

    // Body / bullets outside AC source sections stay description — never invent ACs.
    descriptionLines.push(lines[i]);
  }
...
function buildAnalystOutput(story) {
  ...
  // Explicit JIRA / form AC list only — treat as Acceptance Criteria section, never free-text invent.
  if (!acEntries.length) {
  ...
  // Section headers are audit-only — do not surface as "excluded requirements".
  const displayRejected = allRejected.filter((r) => r.kind !== "section_header");
  if (displayRejected.length) {
    reasoningSteps.push({
      step: "reject_non_ac",
      text: `Excluded ${displayRejected.length} line(s) — metadata, flow, or wrong section: ${displayRejected.map((r) => `"${r.text}"`).join(", ")}`,
      cite: displayRejected.map((r) => r.text).join("; "),
    });
  }

  const ambiguous_acs = [];
  const testable_conditions = [];
  const shallowRejected = [];
  const conceptOwners = []; // { key, acId, text }

  // Rule 1 fallback: unlabeled lines flagged during parsing as having
  // system-normative structure surface here as ambiguous, never invented.
  for (const r of allRejected) {
    if (r?.reason !== UNMARKED_SYSTEM_RULE_REASON) continue;
    ambiguous_acs.push({
      ac_id: null,
      source_line: r.text,
      issue: "Not under a recognized AC section (Business Rules / Alternative Flow / Exception Flow / Acceptance Criteria / synonym) and has no inline AC:/Rule:/Scenario: marker, but reads like a system rule",
      question_for_human: `Should this be formalized as an acceptance criterion under a recognized section? "${r.text.slice(0, 100)}"`,
    });
  }
  ...
  validEntries.forEach((entry, i) => {
    ...
    let assumption = null;
    const conceptKey = acConceptKey(entry.text);
    const dupOf = conceptOwners.find((o) => sameAcConcept(conceptKey, o.key));
    if (dupOf) {
      shallowRejected.push(
        `${entry.text} — duplicate concept of ${dupOf.acId} ("${dupOf.text.slice(0, 60)}")`,
      );
      reasoningSteps.push({
        step: "reject_duplicate_concept",
        text: `${id} "${entry.text}" → rejected (same concept as ${dupOf.acId})`,
        cite: entry.text,
        ac_id: id,
      });
      return;
    ...
    if (vague) {
      ...
      ambiguous_acs.push({
        ...
        issue: `"${word}" is not measurable in the ticket`,
        question_for_human: `What measurable pass/fail evidence defines "${word}" for: ${entry.text.slice(0, 80)}?`,
      });
    }
    const action = inferTestAction(entry.text);
    // No silent drops — shallow lines get an explicit rejected disposition
    if (isShallowTestAction(entry.text, action.summary, entry.section)) {
      shallowRejected.push(`${entry.text} — shallow / not independently testable as stated`);
      reasoningSteps.push({
        step: "reject_shallow",
        text: `${id} "${entry.text}" → rejected (shallow)`,
        cite: entry.text,
        ac_id: id,
      });
      return;
    ...
    if (conceptKey) conceptOwners.push({ key: conceptKey, acId: id, text: entry.text });
  ...
  for (const need of mergedNeeds) {
    ...
    const category = need.id === "target_environment" ? "environment"
      ...
      : "data";
    // Login/UI co-required URLs block the human gate (and design) with credentials.
    const blocks = need.design_blocking ? "design"
      : (category === "environment" || category === "access") ? "execution"
      : undefined;
    if (evidence) {
      blocking.push({
        id: need.id,
      ...
      let gapReason = need.gap_reason;
      if (need.id === "target_environment") {
        const selfUrl = (fullText.match(/https?:\/\/[^\s)>\]]+/gi) || []).find(isTicketSelfReferenceUrl);
        if (selfUrl) {
          gapReason = `Ticket only links to itself (${selfUrl}) — that's a reference to the ticket, not a target test environment.`;
        }
      }
      blocking.push({
        id: need.id,
        ...
        if_not_satisfied: gapReason,
        example_hint: need.hint,
        must_be_provided_by: "human",
      });
      reasoningSteps.push({
        step: "gap",
        text: `${need.label}: ${gapReason} (required for ${need.required_for.join(", ")}).`,
  ...
  // Every line under the ticket's "Pre-conditions" section is a blocking
  // prerequisite (v3: Pre-conditions → PREREQUISITES). State-style lines that
  // carry no concrete value are MISSING, so the human is asked to supply them.
  // Skip lines that duplicate a prerequisite already derived from the ACs
  // (e.g. "User has registered account" ≈ the inferred "Login test user").
  const norm47 = (s) => String(s || "").toLowerCase().slice(0, 47);
  const existingConcepts = new Set(blocking.map((b) => prereqConcept(b.item)).filter(Boolean));
  for (const pc of extractPreConditionSectionLines(fullText)) {
    if (blocking.some((b) => norm47(b.item) === norm47(pc))) continue;
    const concept = prereqConcept(pc);
    if (concept && existingConcepts.has(concept)) continue;
    if (concept) existingConcepts.add(concept);
    const hasConcrete = /[:=@]|\b\d{3,}\b|https?:\/\//i.test(pc);
    blocking.push({
      item: pc,
      category: "data",
      satisfied_by_ticket: hasConcrete,
      if_not_satisfied: hasConcrete
        ? "Pre-condition state must exist before testing."
        : `Pre-condition requires setup before testing: "${pc}"`,
      must_be_provided_by: "human",
      derived_from: "Pre-conditions section",
    });
    reasoningSteps.push({
      step: "precondition",
      text: `Pre-condition "${pc}" → blocking prerequisite (${hasConcrete ? "stated in ticket" : "human must set up state"}).`,
      cite: pc,
    });
  }
  ...
  const rejected_strings = [
    ...allRejected.map((r) => `${r.text} — ${r.reason}`),
    ...shallowRejected,
  ];
  ...
  const ready_for_test_design = testable_conditions.length > 0 && designMissing.length === 0;

  let orchestrator_actions;
  if (!testable_conditions.length) {
    orchestrator_actions = [{
      action: "ASK_HUMAN",
      target: "human",
      detail: "Provide testable Business Rules / Alternative Flow / Exception Flow criteria — zero validated ACs",
      blocking: true,
    }];
  } else if (designMissing.length) {
    orchestrator_actions = designMissing.map((b, i) => {
      const item = b.item || "missing prerequisite";
      const prereq_id = b.id || slugifyPrereqItem(item, i);
      const credLike = /\b(login|credential|password|username|user)\b/i.test(item);
      const detail = credLike
        ? `Provide the username and password of a working test account (${item})`
        : (b.category === "environment" || b.category === "access")
          ? `Provide the full http(s) URL of the target test environment (${item})`
          : `Provide a concrete value for: ${item}`;
      return {
        action: "ASK_HUMAN",
        target: "human",
        detail,
  ...
  return {
    // Legacy adapter for simulator prerequisite panel (must not clobber v3 fields)
    ...legacy,
    success: true,
    scratchpad,
    analyst_reasoning: {
    ...
    analyst_report: {
      what_i_did: [
        "Dispositioned Business Rules / Alternative / Exception lines",
        "Mapped testable conditions with pass/fail evidence",
        "Derived prerequisites from this ticket",
        "Set analysis_complete and ready_for_test_design separately",
      ],
      why: [{
        decision: ready_for_test_design ? "PROCEED to writer" : "ASK_HUMAN before test design",
        reason: ready_for_test_design
          ? `${testable_conditions.length} AC(s); access/env gaps do not block design`
          : (testable_conditions.length ? `${designMissing.length} design-blocking gap(s)` : "zero testable ACs"),
        impact_if_wrong: "Writer invents cases or blocks on execution-only access",
      }],
      orchestrator_actions,
      confidence: {
        overall: testable_conditions.length && !ambiguous_acs.length ? "high" : "medium",
        reason: ambiguous_acs.length ? "vague wording remains" : "stub dispositions from ticket text",
      },
    },
    summary,
  };
...
const UI_TARGET_RE = /\b(login page|sign[\s-]?in page|web\s*page|webpage|browser|navigate|portal|dashboard|click|button|form|screen|ui)\b/i;
...
function inferTestAction(acText) {
  ...
  // UI flows need a concrete page/app URL even when the AC does not name one.
  if (UI_TARGET_RE.test(t)) {
    needs.push("target_environment");
  }
  // Login always co-requires a target URL in the same human gate.
  if (needs.includes("login_user")) {
    needs.push("target_environment");
  }
...
function mergeSetupNeeds(testActions, steps, blob, environmentLine) {
  ...
  // Cross-AC: any login or UI need also requires a target URL for the human gate.
  const storyBlob = [blob, environmentLine, ...(steps || [])].filter(Boolean).join("\n");
  const needsLoginOrUi = needMap.has("login_user")
    || UI_TARGET_RE.test(storyBlob)
    || (testActions || []).some((a) => UI_TARGET_RE.test(a.ac_text || ""));
  if (needsLoginOrUi && !needMap.has("target_environment")) {
    addNeed(
      "target_environment",
      NEED_SPECS.target_environment,
      needMap.has("login_user") ? "login_user" : "UI",
    );
  }
  if (needsLoginOrUi && needMap.has("target_environment")) {
    needMap.get("target_environment").design_blocking = true;
  }
...
function isTicketSelfReferenceUrl(url) {
  return /atlassian\.net\/browse\//i.test(url)
    || /\/browse\/[A-Z][A-Z0-9]*-\d+/i.test(url)
    || /github\.com\/[^/]+\/[^/]+\/issues\/\d+/i.test(url)
    || /\bjira\./i.test(url);
}

function findEnvironmentUrl(fullText) {
  const urls = fullText.match(/https?:\/\/[^\s)>\]]+/gi) || [];
  return urls.find((u) => !isTicketSelfReferenceUrl(u)) || null;
}
...
function findEvidenceInStory(need, fullText, environmentLine, steps) {
  ...
  switch (need.id) {
    ...
    case "target_environment":
      ...
      // A bare link to the ticket itself (JIRA/GitHub issue) is not a target
      // test environment — only count URLs that point somewhere else.
      return findEnvironmentUrl(fullText);
    case "test_file":
...
// Capture EVERY line under a "Pre-conditions" section header, verbatim.
// Unlike parseExplicitPrerequisites (which filters to concrete setup lines via
// isExplicitSetupLine), the ticket's declared pre-conditions are always
// prerequisites per the v3 prompt — including state statements like
// "User has registered account" — so none may be silently dropped.
function extractPreConditionSectionLines(fullText) {
  ...
  for (const raw of lines) {
    ...
    if (!trimmed) continue;
    const sec = getSectionType(trimmed);
    if (sec) {
      inPre = sec === "pre_conditions";
      continue;
    }
    if (!inPre || isMetadataLine(trimmed)) continue;
    const stripped = stripListPrefix(raw) || trimmed;
    if (stripped.length > 2 && !out.some((o) => o.toLowerCase() === stripped.toLowerCase())) {
      out.push(stripped);
    }
  }
  return out;
...
// Collapse a prerequisite line to a coarse concept so semantically-equal
// prerequisites from different sources (a derived "Login test user" and a
// ticket pre-condition "User has registered account") dedupe to one item.
function prereqConcept(text) {
  const t = String(text || "").toLowerCase();
  if (/\b(log(ged)?\s?in|logs? in|login|sign(ed)?\s?in|registered|register|account|credential|test user|valid user|authenticated)\b/.test(t)) {
    return "auth_user";
  }
  if (/\b(environment|staging|production|base ?url|endpoint|https?:\/\/)\b/.test(t)) {
    return "environment";
  }
  return null;
}
...
const AC_STOP = new Set([
  "a", "an", "the", "user", "system", "must", "shall", "can", "with", "when", "for",
  "to", "of", "and", "or", "is", "be", "has", "have", "shows", "show", "enter", "enters",
  "their", "that", "this", "from", "into", "on", "in", "by",
]);

/** Fold synonyms so "invalid password" ≈ "rejects invalid credentials". */
function foldAcToken(t) {
  if (/^(logins?|logs?|signin|signins?|authenticate[ds]?)$/.test(t)) return "auth";
  if (/^(passwords?|credentials?|passwd|secret)$/.test(t)) return "cred";
  if (/^(emails?|usernames?|users?|identity)$/.test(t)) return "identity";
  if (/^(invalid|wrong|bad|reject|rejects|rejected|deny|denies|denied|error|errors|fail|fails|failure)$/.test(t)) return "neg";
  if (/^(valid|correct|success|successful|allow|allows|accepted|accepts)$/.test(t)) return "pos";
  if (/^(tokens?|otps?|codes?)$/.test(t)) return "token";
  return t;
}

/** Coarse concept fingerprint for AC dedupe (same idea → one testable AC). */
function acConceptKey(text) {
  const tokens = String(text || "")
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, " ")
    .split(/\s+/)
    .filter((t) => t.length > 2 && !AC_STOP.has(t))
    .map(foldAcToken);
  return [...new Set(tokens)].sort().join("|");
}

function sameAcConcept(keyA, keyB) {
  ...
  // Same polarity + shared domain (e.g. neg+cred)
  if (sa.has("neg") && sb.has("neg") && inter >= 2) return true;
  if (sa.has("pos") && sb.has("pos") && inter >= 2) return true;
  return jaccard >= 0.6;
}
...
function validateAnalystOutput(story, analystOutput) {
  ...
  const entries = (story?.acceptance_criteria_entries || rawAcs.map((text) => ({ text, section: "ac" })));
  ...
  if (!Array.isArray(conditions)) {
    ...
    if (analystOutput?.ready_for_test_design === true && conditions.length === 0) {
      failures.push("ready_for_test_design is true but testable_conditions is empty");
      failedRules.add(ruleTestable);
    }
  }

  // Disposition coverage — every source AC line must be testable | ambiguous | unimplemented | rejected
  {
    const ruleDisposition = "No silent drops — every BR/Alt/Exception line needs an explicit disposition";
    const expectedLines = (story?.acceptance_criteria_entries || []).length
      ? (story.acceptance_criteria_entries || []).map((e) => stripListPrefix(typeof e === "string" ? e : e?.text)).filter((t) => t && t.length >= 3)
      : (story?.acceptance_criteria_list || []).map((t) => stripListPrefix(t)).filter((t) => t && t.length >= 3);
    const conditionList = Array.isArray(analystOutput?.testable_conditions) ? analystOutput.testable_conditions : [];
    const dispositionTexts = [];
    for (const c of conditionList) {
      if (c?.ac_text) dispositionTexts.push(c.ac_text);
    }
    const ar = analystOutput?.analyst_reasoning || {};
    for (const a of ar.ambiguous_acs || []) {
      if (a?.source_line) dispositionTexts.push(a.source_line);
      else if (a?.ac_text) dispositionTexts.push(a.ac_text);
      else if (a?.ac_id) {
        const hit = conditionList.find((x) => x?.id === a.ac_id);
        if (hit?.ac_text) dispositionTexts.push(hit.ac_text);
      }
      const q = String(a?.question_for_human || "");
      const m = q.match(/\bfor:\s*(.+)$/i);
      if (m) dispositionTexts.push(m[1].replace(/\?+\s*$/, "").trim());
    }
    for (const u of ar.unimplemented_rules || []) {
      dispositionTexts.push(typeof u === "string" ? u : u?.text || "");
    }
    for (const r of ar.rejected_as_non_ac || []) {
      if (typeof r === "string") dispositionTexts.push(r.split(/\s+[—–-]\s+/)[0]);
      else if (r?.text) dispositionTexts.push(r.text);
    }
    const norms = dispositionTexts
      .map((t) => stripListPrefix(t).replace(/\s+/g, " ").trim().toLowerCase())
      .filter((t) => t.length >= 3);
    const uncovered = [];
    for (const line of expectedLines) {
      const n = stripListPrefix(line).replace(/\s+/g, " ").trim().toLowerCase();
      const covered = norms.some((d) => d === n || (n.length >= 12 && d.includes(n)) || (d.length >= 12 && n.includes(d)));
      if (!covered) uncovered.push(line);
    }
    if (uncovered.length) {
      const preview = uncovered.slice(0, 5).map((l) => `"${String(l).slice(0, 80)}"`).join(", ");
      failures.push(`DISPOSITION: ${uncovered.length}/${expectedLines.length} source line(s) have no disposition — ${preview}`);
      failedRules.add(ruleDisposition);
    }
  }
  ...
  if (!Array.isArray(gaps)) {
    failures.push("coverage_gaps must be an array (may be empty)");
  ...
  if (typeof analystOutput?.analysis_complete !== "boolean") {
    failures.push("analysis_complete must be a boolean");
    failedRules.add(ruleTestable);
  }

  // MAIN GATE — keep aligned with agents/analyst-contract.js
  const ruleMainGate = "MAIN GATE: Validator second-opinions Analyst readiness proposal";
  const actions = analystOutput?.analyst_report?.orchestrator_actions || [];
  const missingBlocking = (analystOutput?.prerequisites_needed?.blocking || [])
  ...
  const conf = String(analystOutput?.analyst_report?.confidence?.overall || "").toLowerCase();
  const vagueAskRe = /\b(need more info|more information|clarify|unclear|tbd|todo|n\/a|please clarify|not (enough|clear)|requirements?\s+unclear)\b/i;
  if (!Array.isArray(actions) || !actions.length) {
    failures.push("MAIN GATE: orchestrator_actions must be non-empty (Analyst readiness proposal required)");
    failedRules.add(ruleMainGate);
  }
  ...
  if (analystOutput?.ready_for_test_design === true && analystOutput?.analysis_complete === false) {
    failures.push("MAIN GATE: ready_for_test_design true requires analysis_complete true");
    failedRules.add(ruleMainGate);
  }
  if (analystOutput?.ready_for_test_design === true && !hasProceed) {
    failures.push("MAIN GATE: ready_for_test_design true requires a PROCEED action");
    failedRules.add(ruleMainGate);
  }
  if (hasProceed && analystOutput?.ready_for_test_design !== true) {
  ...
  if (analystOutput?.ready_for_test_design === true && designMissing.length) {
    failures.push("MAIN GATE: ready_for_test_design true while design-blocking prerequisites are missing");
    failedRules.add(ruleMainGate);
  }
  if (missingBlocking.length && !blockingActs.length && !hasProceed) {
  ...
  for (const a of actions) {
    ...
    const vague = d.length < 16 || vagueAskRe.test(d)
      || !/\b(url|uri|credential|password|token|api|curl|env|environment|staging|uat|role|account|username|confirm|provide|supply|decision|ticket|id)\b/i.test(d);
    if (vague) {
      failures.push(`MAIN GATE: vague ASK_HUMAN rejected (escalate with a concrete artifact) — "${d.slice(0, 80)}"`);
      failedRules.add(ruleMainGate);
    }
  }
