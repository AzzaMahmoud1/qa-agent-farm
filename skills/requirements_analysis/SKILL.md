...
description: >-
  Analyze one JIRA-style ticket and decide what is testable before any test
  design starts — extract testable conditions, prerequisites, and coverage
  gaps, and emit a readiness verdict (PROCEED / ASK_HUMAN / HOLD /
  FETCH_DEPENDENCY / RETRY_WITH_INFO). Ported from
  src/prompts/agent1_requirement_analyst_v3.md; output validated by
  analyst_agent/validation.py against analyst_agent/models.py.
---
...
```json block. The schema below is enforced by `analyst_agent/models.py`
(`RequirementsAnalysisResult`) — see `schemas/output.schema.json` in this
folder for the JSON Schema form.
...
This skill is a direct port of `src/prompts/agent1_requirement_analyst_v3.md`
(the JS pipeline's existing "Agent 1" prompt) into this standalone Python
component's skill format. The two should be kept in sync if either changes —
this is a parallel component, not a replacement.
